...
CREATE TABLE IF NOT EXISTS `MeuPDV`.`HistoricoLogin` (
  ...
  `token_hmac` VARCHAR(25) NOT NULL,
...
-- -----------------------------------------------------
-- Table `MeuPDV`.`EntregaProduto`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `MeuPDV`.`EntregaProduto` ;

CREATE TABLE IF NOT EXISTS `MeuPDV`.`EntregaProduto` (
  `id_Entrega` INT NOT NULL,
  `id_Prod` INT NOT NULL,
  `validade` DATE NOT NULL,
  PRIMARY KEY (`id_Entrega`, `id_Prod`),
  INDEX `id_Prod_idx` (`id_Prod` ASC),
  CONSTRAINT `id_Entrega`
    FOREIGN KEY (`id_Entrega`)
    REFERENCES `MeuPDV`.`Entrega` (`id_Entrega`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `id_Prod`
    FOREIGN KEY (`id_Prod`)
...
DELIMITER $$
USE `MeuPDV`$$
CREATE PROCEDURE `Register` (pass varchar(50), username varchar(50), nome varchar(75), id_contato int) 
BEGIN
	SET @salt = LEFT(UUID(), 8);
    SET @hashedPass = SHA2(CONCAT(@salt, pass),0);
    INSERT INTO Usuario(nome_Usuario, username_Usuario, senha_Usuario, salt_Usuario, id_Contato) 
			VALUES (nome, username, @hashedPass, @salt, id_contato);
END;$$
...
-- -----------------------------------------------------
-- procedure NovaEntrega
-- -----------------------------------------------------

USE `MeuPDV`;
DROP procedure IF EXISTS `MeuPDV`.`NovaEntrega`;

DELIMITER $$
USE `MeuPDV`$$
CREATE PROCEDURE `NovaEntrega` ()
BEGIN
	INSERT INTO Entrega(id_Prog)
    VALUES();
END$$
...
USE `MeuPDV`$$
CREATE DEFINER = CURRENT_USER TRIGGER `MeuPDV`.`Produto_BEFORE_INSERT` BEFORE INSERT ON `Produto` FOR EACH ROW
BEGIN
...
USE `MeuPDV`$$
DROP TRIGGER IF EXISTS `MeuPDV`.`EntregaProduto_BEFORE_INSERT` $$
USE `MeuPDV`$$
CREATE DEFINER = CURRENT_USER TRIGGER `MeuPDV`.`EntregaProduto_BEFORE_INSERT` BEFORE INSERT ON `EntregaProduto` FOR EACH ROW
BEGIN
	SET @val = NEW.validade;
	IF @val <= CURDATE() THEN
		SELECT "VALIDADE_INCORRETA";
	END IF;
END$$
...
START TRANSACTION;
USE `MeuPDV`;
INSERT INTO `MeuPDV`.`Fornecedor` (`id_Forn`, `nome_Forn`, `Contato_id_Contato`) VALUES (1, 'Elmachips', 1);
INSERT INTO `MeuPDV`.`Fornecedor` (`id_Forn`, `nome_Forn`, `Contato_id_Contato`) VALUES (2, 'Coca-Cola', 2);
INSERT INTO `MeuPDV`.`Fornecedor` (`id_Forn`, `nome_Forn`, `Contato_id_Contato`) VALUES (3, 'Jair-Frutas', 1);
...
START TRANSACTION;
USE `MeuPDV`;
INSERT INTO `MeuPDV`.`Produto` (`id_Prod`, `id_Forn`, `nome_Prod`, `cdgBarras_Prod`, `val_Prod`, `quant_Prod`) VALUES (1, 1, 'Cheetos', 01234567800, 2.50, 300);
INSERT INTO `MeuPDV`.`Produto` (`id_Prod`, `id_Forn`, `nome_Prod`, `cdgBarras_Prod`, `val_Prod`, `quant_Prod`) VALUES (2, 2, 'Coca-Cola', 01134567800, 5.00, 100);
INSERT INTO `MeuPDV`.`Produto` (`id_Prod`, `id_Forn`, `nome_Prod`, `cdgBarras_Prod`, `val_Prod`, `quant_Prod`) VALUES (3, 3, 'Maça o kilo', 29923928394, 7.79, 144);
...
-- -----------------------------------------------------
-- Data for table `MeuPDV`.`Entrega`
-- -----------------------------------------------------
START TRANSACTION;
USE `MeuPDV`;
INSERT INTO `MeuPDV`.`Entrega` (`id_Entrega`, `data_Entrega`, `quant_Entrega`, `custo_Entrega`) VALUES (1, 'NOW()', 100, 540);
INSERT INTO `MeuPDV`.`Entrega` (`id_Entrega`, `data_Entrega`, `quant_Entrega`, `custo_Entrega`) VALUES (2, 'NOW()', 30, 200);

COMMIT;


-- -----------------------------------------------------
-- Data for table `MeuPDV`.`EntregaProduto`
-- -----------------------------------------------------
START TRANSACTION;
USE `MeuPDV`;
INSERT INTO `MeuPDV`.`EntregaProduto` (`id_Entrega`, `id_Prod`, `validade`) VALUES (1, 2, '01-05-2018');
INSERT INTO `MeuPDV`.`EntregaProduto` (`id_Entrega`, `id_Prod`, `validade`) VALUES (2, 1, '15-03-2019');
